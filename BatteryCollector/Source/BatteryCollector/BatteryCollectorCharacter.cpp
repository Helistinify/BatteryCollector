...
ABatteryCollectorCharacter::ABatteryCollectorCharacter()
{
	...
	//Set Initail Power and character power
	InitialPower = 2000.f;
	CharacterPower = InitialPower;

	// set dependence of speed on the power lvl
	SpeedFactor = 0.75f;
	BaseSpeed = 10.0f;
...
{
	ABatteryCollectorGameMode* CurrGameMode = (ABatteryCollectorGameMode*)GetWorld()->GetAuthGameMode();
	if (CurrGameMode->GetCurrentState() == EBAtteryPlayState::EPlaying) {
		// Get overalpping actors
		...
		// For each actor in array check if it's pickup
		for (int32 iCollected = 0; iCollected < CollectedActors.Num(); ++iCollected)
		{
			...
			if (TestPickup && !TestPickup->IsPendingKill() && TestPickup->IsActive()) {
				// Call WasCollected of the pickup
				TestPickup->WasCollected();
				// Check to see if the pickup is battery
...
// Update current power of the character, Called when power is changed
